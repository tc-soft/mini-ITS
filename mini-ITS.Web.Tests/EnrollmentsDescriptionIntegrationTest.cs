...
namespace mini_ITS.Web.Tests
{
    public class EnrollmentsDescriptionIntegrationTest
    {
        ...
        protected async Task<HttpResponseMessage> EditPutAsync(EnrollmentsDescriptionDto enrollmentsDescriptionDto)
        {
            var response = await TestClient.PutAsJsonAsync($"{ApiRoutes.EnrollmentsDescription.Edit}/{enrollmentsDescriptionDto.Id}", enrollmentsDescriptionDto);

            return response;
        }
        protected async Task<HttpResponseMessage> DeleteAsync(Guid id)
        {
            var response = await TestClient.DeleteAsync($"{ApiRoutes.EnrollmentsDescription.Delete}/{id}");
            return response;
        }
    }
}