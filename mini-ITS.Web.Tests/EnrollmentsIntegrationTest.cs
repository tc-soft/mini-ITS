...
namespace mini_ITS.Web.Tests
{
    public class EnrollmentsIntegrationTest
    {
        ...
        protected async Task<HttpResponseMessage> IndexAsync(SqlPagedQuery<EnrollmentsDto> sqlPagedQuery)
        {
            ...
            var response = await TestClient.GetAsync($"{ApiRoutes.Enrollments.Index}?{queryString.Result}");

            return response;
        }
        protected async Task<HttpResponseMessage> CreateAsync(EnrollmentsDto enrollmentsDto)
        {
            var response = await TestClient.PostAsJsonAsync(ApiRoutes.Enrollments.Create, enrollmentsDto);

            return response;
        }
        protected async Task<HttpResponseMessage> EditGetAsync(Guid id)
        {
            var response = await TestClient.GetAsync($"{ApiRoutes.Enrollments.Edit}/{id}");

            return response;
        }
        protected async Task<HttpResponseMessage> EditPutAsync(EnrollmentsDto enrollmentsDto)
        {
            var response = await TestClient.PutAsJsonAsync($"{ApiRoutes.Enrollments.Edit}/{enrollmentsDto.Id}", enrollmentsDto);

            return response;
        }
        protected async Task<HttpResponseMessage> DeleteAsync(Guid id)
        {
            var response = await TestClient.DeleteAsync($"{ApiRoutes.Enrollments.Delete}/{id}");
